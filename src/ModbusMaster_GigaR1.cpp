...
/*
  ...
  ModbusMaster_GigaR1.cpp - Arduino library for communicating with Modbus slaves
  over RS232/485 (via RTU protocol). Optimized for Arduino Giga R1.

  Library:: ModbusMaster_GigaR1
...
#define ku16MBInterFrameDelay 10  // ms
...
Creates class object; initialize it using ModbusMaster_GigaR1::begin().

@ingroup setup
*/
ModbusMaster_GigaR1::ModbusMaster_GigaR1(void)
{
  _idle = 0;
  _preTransmission = 0;
  _postTransmission = 0;
...
@ingroup setup
*/
void ModbusMaster_GigaR1::begin(uint8_t slave, Stream &serial)
{
...
void ModbusMaster_GigaR1::beginTransmission(uint16_t u16Address)
{
...
uint8_t ModbusMaster_GigaR1::requestFrom(uint16_t address, uint16_t quantity)
{
  uint8_t read;
...
void ModbusMaster_GigaR1::sendBit(bool data)
{
...
void ModbusMaster_GigaR1::send(uint16_t data)
{
...
void ModbusMaster_GigaR1::send(uint32_t data)
{
  send(lowWord(data));
  send(highWord(data));
}
...
void ModbusMaster_GigaR1::send(uint8_t data)
{
  send(word(data));
}
...
uint8_t ModbusMaster_GigaR1::available(void)
{
  return _u8ResponseBufferLength - _u8ResponseBufferIndex;
}


uint16_t ModbusMaster_GigaR1::receive(void)
{
...
@see ModbusMaster_GigaR1::ModbusMasterTransaction()
*/
void ModbusMaster_GigaR1::idle(void (*idle)())
{
  _idle = idle;
}
...
@see ModbusMaster_GigaR1::ModbusMasterTransaction()
@see ModbusMaster_GigaR1::postTransmission()
*/
void ModbusMaster_GigaR1::preTransmission(void (*preTransmission)())
{
...
@see ModbusMaster_GigaR1::ModbusMasterTransaction()
@see ModbusMaster_GigaR1::preTransmission()
*/
void ModbusMaster_GigaR1::postTransmission(void (*postTransmission)())
{
...
@see ModbusMaster_GigaR1::clearResponseBuffer()
@param u8Index index of response buffer array (0x00..0x3F)
@return value in position u8Index of response buffer (0x0000..0xFFFF)
@ingroup buffer
*/
uint16_t ModbusMaster_GigaR1::getResponseBuffer(uint8_t u8Index)
{
...
@see ModbusMaster_GigaR1::getResponseBuffer(uint8_t u8Index)
@ingroup buffer
*/
void ModbusMaster_GigaR1::clearResponseBuffer()
{
  uint8_t i;
...
@see ModbusMaster_GigaR1::clearTransmitBuffer()
@param u8Index index of transmit buffer array (0x00..0x3F)
@param u16Value value to place in position u8Index of transmit buffer (0x0000..0xFFFF)
@return 0 on success; exception number on failure
@ingroup buffer
*/
uint8_t ModbusMaster_GigaR1::setTransmitBuffer(uint8_t u8Index, uint16_t u16Value)
{
  if (u8Index < ku8MaxBufferSize)
  {
...
@see ModbusMaster_GigaR1::setTransmitBuffer(uint8_t u8Index, uint16_t u16Value)
@ingroup buffer
*/
void ModbusMaster_GigaR1::clearTransmitBuffer()
{
  uint8_t i;
...
@ingroup discrete
*/
uint8_t ModbusMaster_GigaR1::readCoils(uint16_t u16ReadAddress, uint16_t u16BitQty)
{
...
@ingroup discrete
*/
uint8_t ModbusMaster_GigaR1::readDiscreteInputs(uint16_t u16ReadAddress,
  uint16_t u16BitQty)
{
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16BitQty;
...
@ingroup register
*/
uint8_t ModbusMaster_GigaR1::readHoldingRegisters(uint16_t u16ReadAddress,
  uint16_t u16ReadQty)
{
  _u16ReadAddress = u16ReadAddress;
...
@ingroup register
*/
uint8_t ModbusMaster_GigaR1::readInputRegisters(uint16_t u16ReadAddress,
  uint8_t u16ReadQty)
{
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16ReadQty;
...
@ingroup discrete
*/
uint8_t ModbusMaster_GigaR1::writeSingleCoil(uint16_t u16WriteAddress, uint8_t u8State)
{
...
@ingroup register
*/
uint8_t ModbusMaster_GigaR1::writeSingleRegister(uint16_t u16WriteAddress,
...
@ingroup discrete
*/
uint8_t ModbusMaster_GigaR1::writeMultipleCoils(uint16_t u16WriteAddress,
  uint16_t u16BitQty)
{
  _u16WriteAddress = u16WriteAddress;
...
uint8_t ModbusMaster_GigaR1::writeMultipleCoils()
{
...
@ingroup register
*/
uint8_t ModbusMaster_GigaR1::writeMultipleRegisters(uint16_t u16WriteAddress,
...
uint8_t ModbusMaster_GigaR1::writeMultipleRegisters()
{
...
@ingroup register
*/
uint8_t ModbusMaster_GigaR1::maskWriteRegister(uint16_t u16WriteAddress,
...
@ingroup register
*/
uint8_t ModbusMaster_GigaR1::readWriteMultipleRegisters(uint16_t u16ReadAddress,
...
@return 0 on success; exception number on failure
*/
uint8_t ModbusMaster_GigaR1::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  uint8_t u8MBStatus = ku8MBInvalidCRC;
  uint8_t u8RetryCount = 0;
  


  // Retry loop for Giga R1 compatibility
  while (u8RetryCount < ku8MBMaxRetries && u8MBStatus != ku8MBSuccess)  {
    ...
    // Add delay before transmission for Giga R1


    // transmit request
    if (_preTransmission)    {
          delay(ku16MBInterFrameDelay);
      _preTransmission();
    }
    
    // Transmit all bytes at once for better timing
    for (i = 0; i < u8ModbusADUSize; i++)
    {
    ...
    // Add delay after transmission for Giga R1
    delay(ku16MBInterFrameDelay);
    
    if (_postTransmission)    {
       delayMicroseconds(100);  // <-- necesario en Giga R1 para evitar truncamiento
      _postTransmission();
    }
    
    // Add delay before receiving for Giga R1
    delay(ku16MBReceiveDelay);
    ...
    while (u8BytesLeft && !u8MBStatus)
    {
...
#endif
        // Reset timeout for each byte received
        u32StartTime = millis();
      }      else      {
#if __MODBUSMASTER_DEBUG__
      ...
      // Check for timeout with byte-level timeout for Giga R1
      if ((millis() - u32StartTime) > ku16MBByteTimeout)
      {
    ...
    if (!u8MBStatus && u8ModbusADUSize >= 5)
    {
      ...
      // 🔍 Mostrar la trama recibida sin importar si es válida
    ...
    // If transaction failed, increment retry counter and try again
    if (u8MBStatus != ku8MBSuccess)
    {
      u8RetryCount++;
      if (u8RetryCount < ku8MBMaxRetries)
      {
        delay(ku16MBInterFrameDelay * u8RetryCount); // Exponential backoff
      }
    }
  }
